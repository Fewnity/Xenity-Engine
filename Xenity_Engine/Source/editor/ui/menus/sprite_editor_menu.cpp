...
void SpriteEditorMenu::AddNewSpriteSelection(Vector2 position, Vector2 size, Vector2 pivot)
{
	SpriteSelection selection;
...
void SpriteEditorMenu::Draw()
{
	...
	if (visible)
	{
		...
		float oldCursorXPos = ImGui::GetCursorPosX();
		float oldCursorYPos = ImGui::GetCursorPosY();

		ImVec2 availSize = ImGui::GetContentRegionAvail();
		if (spriteToEdit)
		{
			ImVec2 winPos = ImGui::GetWindowPos();
			float topX = winPos.x - (spriteToEdit->GetWidth() * zoom) / 2.0f + availSize.x / 2.0f + oldCursorXPos + spriteToEdit->GetWidth() * offset.x * zoom;
			float topY = winPos.y - (spriteToEdit->GetHeight() * zoom) / 2.0f + availSize.y / 2.0f + oldCursorYPos + spriteToEdit->GetHeight() * offset.y * zoom;
			float bottomX = winPos.x + (spriteToEdit->GetWidth() * zoom) / 2.0f + availSize.x / 2.0f + oldCursorXPos + spriteToEdit->GetWidth() * offset.x * zoom;
			float bottomY = winPos.y + (spriteToEdit->GetHeight() * zoom) / 2.0f + availSize.y / 2.0f + oldCursorYPos + spriteToEdit->GetHeight() * offset.y * zoom;
			...
			for (int selectionIndex = 0; selectionIndex < spriteSelectionCount; selectionIndex++)
			{
				...
				float lineRectTopX = topX + (currentSelection.position.x * 0.5f) * zoom;
				float lineRectTopY = topY + (currentSelection.position.y * 0.5f) * zoom;
				float lineRectBottomX = topX + (currentSelection.position.x * 0.5f + currentSelection.size.x) * zoom;
				float lineRectBottomY = topY + (currentSelection.position.y * 0.5f + currentSelection.size.y) * zoom;
	...
	if (spriteToEdit)
	{
		...
		for (int selectionIndex = 0; selectionIndex < spriteSelectionCount; selectionIndex++)
		{
			...
			ImGui::Image((ImTextureID)spriteToEdit->GetTextureId(), ImVec2(150, 150));
			std::string buttonText = "Select" + EditorUI::GenerateItemId();
			...
			if (currentSelectedSpriteIndex == selectionIndex)
			{
				...
				bool positionUpdated = EditorUI::DrawInput("Position", selectionPosition);
				if (positionUpdated)
					currentSelection.position = selectionPosition;

				Vector2 selectionSize = currentSelection.size;
				bool sizeUpdated = EditorUI::DrawInput("Size", selectionSize);
				if (sizeUpdated)
					currentSelection.size = selectionSize;

				Vector2 selectionPivot = currentSelection.pivot;
				bool pivotUpdated = EditorUI::DrawInput("Pivot", selectionPivot);
				if (pivotUpdated)
