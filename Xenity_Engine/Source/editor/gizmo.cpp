...
void Gizmo::DrawBillboard(const Vector3& position, const Vector2& scale, const std::shared_ptr<Texture>& texture, const Color& color)
{
	...
	SpriteManager::DrawSprite(position, Graphics::usedCamera->GetTransform()->GetEulerAngles(), Vector3(0.2f), Color::CreateFromRGBAFloat(rgba.r, rgba.g, rgba.b, alpha), *AssetManager::unlitMaterial, texture);
}
