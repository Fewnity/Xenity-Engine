...
std::string modelsPath = R"(OpenGL_Test\Source\models\)"; //TODO remove this
...
std::string finalpath = PROJECT_FOLDER + modelsPath;
	...
	while (std::getline(file, line)) {
		...
		if (line.substr(0, 2) == "v ") {
			...
			vertexIndices.emplace_back(v1);
			vertexIndices.emplace_back(v2);
			vertexIndices.emplace_back(v3);

			textureIndices.emplace_back(vt1);
			textureIndices.emplace_back(vt2);
			textureIndices.emplace_back(vt3);

			normalsIndices.emplace_back(vn1);
			normalsIndices.emplace_back(vn2);
			normalsIndices.emplace_back(vn3);
		}
		text += line + '\n';
	}
